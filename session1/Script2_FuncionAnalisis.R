...
datos <-data.frame(y=trees$Girth, x1=trees$Height, x2=trees$Volume)
...
# El mejor modelo es el model5, ya que tiene le mayor adjusted R² y esta es una métrica a maximizar
...
# a) El mejor modelo es el model5
...
# b)
# El mejor es con las variables x1 y x2, ya que su modelo posee el mayor r² ajustado

summary(lm(y~bs(x1)+bs(x2), data= datos))
summary(lm(y~bs(x1)+bs(x3), data= datos))
summary(lm(y~bs(x2)+bs(x3), data= datos))
...
datos <-data.frame( y=iris$Sepal.Width,
                    ...
                    x2=iris$Petal.Length,
                    x3=iris$Petal.Width)
# El mejor modelo es el model3, ya que tiene el mayor R² ajustado, ya que esa métrica es a maximizar.
...
# Es falso, el model3 generaliza mejor

summary(gam(y~ns(x1,16)+ns(x2,16)+ns(x3,16), data= datos))
summary(gam(y~ns(x1,16), data= datos))
